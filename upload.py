...
def get_arguments() -> dict:
    ...
    while True:
        local_path_str = input('<将所需上传的项目目录（不是Res目录。例如：202410181422_C22091401_A_PRM32502260044_SEQ_A_AB_PE150）拖曳到此处后按回车，直接回车结束输入> ')
    ...
    while True:
        if 'machine_id' in arguments_dict.keys():
            temp = input(f"<直接按回车使用{arguments_dict['machine_id']} 或手动输入机器号（测试目的请输入99999）> ")
            if temp == '':
